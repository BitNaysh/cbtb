...
import { CheckBox, ChevronLeft, ChevronRight, UploadFile } from "@mui/icons-material";
import axios from "axios";
import { Combobox } from "@headlessui/react";

const people = ["Database 1", "Database 2", "Database 3", "Database 4", "Database 5"];
...
const Dashboard = () => {
    ...
    const handelSubmit = async () => {
        ...
        setIsLoading(true);
        const res = await axios.post("http://localhost:8000/api/compare_face/media/", _file, {
            headers: {
                "content-type": "multipart/form-data",
            },
        });
        if (res) {
            ...
            set_img_url_match("http://localhost:8000" + match.image_url);
            setIsLoading(false);
            setShowResult(true);
        }
    };
